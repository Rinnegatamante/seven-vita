/* main.c -- 7 Billion Humans .so loader
 *
...
static so_default_dynlib default_dynlib[] = {
	{ "SDL_utf8strlcpy", (uintptr_t)&SDL_utf8strlcpy },
...
void patch_game(void) {
	...
	hook_addr(so_symbol(&hrm_mod, "_Z28android_adjustThreadAffinityv"), ret0);
...
int main(int argc, char *argv[]) {
	...
	printf("Loading lib7BillionHumans\n");
	if (so_file_load(&hrm_mod, DATA_PATH "/lib7BillionHumans.so", LOAD_ADDRESS) < 0)
		fatal_error("Error could not load %s.", DATA_PATH "/lib7BillionHumans.so");
	so_relocate(&hrm_mod);
